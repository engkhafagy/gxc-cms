...
class RGenerator extends CApplicationComponent
{
    ...
    * Modified by Sayyed to enable generate Auth actions for both back and front ends.
	*/
	protected function getAllControllers()
	{
		$basePath = Yii::app()->basePath;             //Backend
        $frontEnd = Yii::getPathOfAlias('frontend'); //Frontend 
		$items['controllers'] = $this->getControllersInPath($basePath.DIRECTORY_SEPARATOR.'controllers');
        $frontend['controllers'] = $this->getControllersInPath($frontEnd.DIRECTORY_SEPARATOR.'controllers');
        $items['controllers'] = array_merge($items['controllers'], $frontend['controllers']);
		$items['modules'] = $this->getControllersInModules($basePath);
        $frontend['modules'] = $this->getControllersInModules($frontEnd);
        $items['modules'] = array_merge($items['modules'], $frontend['modules']);
		return $items;
	}
